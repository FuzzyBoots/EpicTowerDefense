    ...
    public sealed class PreviewManager
    {
        ...
        public static async Task<bool> Create(AssetInfo info, string sourcePath = null, Action onCreated = null)
        {
            ...
            else
            {
                ...
                // remove ~ from directory names as these files will be ignored by the asset library
                sourcePath = sourcePath.Replace("~", "");
                RenameSubdirectories(UnityPreviewGenerator.GetPreviewWorkFolder());
        ...
        private static void RenameSubdirectories(string rootPath)
        {
            string[] subdirs = Directory.GetDirectories(rootPath);
            foreach (string subdir in subdirs)
            {
                string name = Path.GetFileName(subdir);
                string newPath = subdir;
                if (name.EndsWith("~"))
                {
                    string newName = name.TrimEnd('~');
                    newPath = Path.Combine(Path.GetDirectoryName(subdir), newName);
                    if (!Directory.Exists(newPath))
                    {
                        Directory.Move(subdir, newPath);
                    }
                }
                RenameSubdirectories(newPath);
            }
        }
